...
import (
	...
	evm_types "github.com/fibonacci-chain/fbc/x/evm/types"
	"github.com/fibonacci-chain/fbc/x/gov"
	"github.com/fibonacci-chain/fbc/x/gov/types"
	"github.com/fibonacci-chain/fbc/x/staking"
	staking_keeper "github.com/fibonacci-chain/fbc/x/staking/keeper"
	staking_types "github.com/fibonacci-chain/fbc/x/staking/types"
		...
		},
		{
			...
			func() {
				...
				for i := 0; i < 100; i++ {
					...
					//fmt.Println("minter reward: ", minter.MintedPerBlock) //1fibo per block common pool 0.2
				}
				commision := suite.app.DistrKeeper.GetValidatorAccumulatedCommission(suite.ctx, valopaddress)
				suite.Require().True(commision.IsEqual(sdk.NewDecCoins(sdk.NewDecCoinFromDec(sdk.DefaultBondDenom, sdk.NewDecWithPrec(98, 0)))))
