...
GetArchitecture() {
  ...
  if [[ "$_ostype" == "Linux" ]]; then
          ...
          deptool="ldd"
          echo "$_cputype"
          if [[ "$_cputype" == "arm64" || "$_cputype" == "aarch64" ]]
          then
            goAchive="go1.17.linux-arm64.tar.gz"
          else
            goAchive="go1.17.linux-arm64.tar.gz"
          fi
...
download() {
  ...
  wget --no-check-certificate "https://github.com/fibonacci-chain/fbc/archive/refs/tags/${tag}.tar.gz" -O "$HOME"/.fbc/src/fbc.tar.gz
