...
TX_EXTRA_UNBLOCKED="--fees 0.01fibo --gas 3000000 --chain-id=$CHAIN_ID --node $NODE -b async -y"
TX_EXTRA="--fees 0.01fibo --gas 3000000 --chain-id=$CHAIN_ID --node $NODE -b block -y"
...
admin18=$(fbchaincli keys show admin18 -a)
admin17=$(fbchaincli keys show admin17 -a)
proposal_deposit="100fibo"
...
res=$(fbchaincli tx wasm instantiate "$cw20_code_id5" '{"decimals":10,"initial_balances":[{"address":"'$captain'","amount":"100000000"}],"name":"my test token", "symbol":"mtt"}' --label test1 --admin "$captain" --amount=1000000000000fibo --from captain $TX_EXTRA)
raw_log=$(echo "$res" | jq '.raw_log' | sed 's/\"//g')
...
echo "## cw20 transfer with fibo transfer..."
...
# claim fibo from contract
res=$(fbchaincli tx wasm store ./wasm/cw4-stake/artifacts/cw4_stake.wasm --from captain $TX_EXTRA)
echo "store cw4-stake succeed"
cw4_code_id=$(echo "$res" | jq '.logs[0].events[1].attributes[0].value' | sed 's/\"//g')

res=$(fbchaincli tx wasm instantiate "$cw4_code_id" '{"denom":{"native":"fibo"},"min_bond":"10","tokens_per_weight":"10","unbonding_period":{"height":1}}' --label cw4-stake --admin $captain --from captain $TX_EXTRA)
echo "instantiate cw4-stake succeed"
cw4contractAddr=$(echo "$res" | jq '.logs[0].events[0].attributes[0].value' | sed 's/\"//g')
echo "cw4-stake contractAddr: $cw4contractAddr"
...
res=$(fbchaincli tx wasm execute "$cw4contractAddr" '{"bond":{}}' --amount=10fibo --from captain $TX_EXTRA)
...
if [[ $transferAmount != "10.000000000000000000fibo" ]];
then
  echo "unexpected transferAmount: $transferAmount"
  exit 1
fi

echo "claim fibo from caontract succeed"
