...
echo "0-----------------------"
fbchaincli tx send captain $(fbchaincli keys show fred -a) 1000fibo --fees 0.001fibo -y -b block

echo "1-----------------------"
echo "## Add new CosmWasm contract"
RESP=$(fbchaincli tx wasm store "$DIR/../../../x/wasm/keeper/testdata/hackatom.wasm" \
  --from captain --fees 0.001fibo --gas 1500000 -y --node=http://localhost:26657 -b block -o json)
...
fbchaincli tx wasm instantiate "$CODE_ID" "$INIT" --admin="$(fbchaincli keys show captain -a)" \
  --from captain  --fees 0.001fibo --amount="100fibo" --label "local0.1.0" \
...
fbchaincli tx wasm execute "$CONTRACT" "$MSG" \
  --from captain \
  --gas 1000000 --fees 0.001fibo -y  -b block -o json | jq
...
fbchaincli tx wasm set-contract-admin "$CONTRACT" "$(fbchaincli keys show fred -a)" \
  --from captain --fees 0.001fibo -y -b block -o json | jq
...
RESP=$(fbchaincli tx wasm store "$DIR/../../../x/wasm/keeper/testdata/burner.wasm" \
  --from captain --fees 0.001fibo --gas 1000000 -y --node=http://localhost:26657 -b block -o json)
...
fbchaincli tx wasm migrate "$CONTRACT" "$BURNER_CODE_ID" "{\"payout\": \"$DEST_ACCOUNT\"}" --from fred  --fees 0.001fibo \
 -b block -y -o json | jq
...
echo "### Query old admin: $(fbchaincli q wasm contract "$CONTRACT" -o json | jq -r '.contract_info.admin')"
echo "### Update contract"
fbchaincli tx wasm clear-contract-admin "$CONTRACT" --fees 0.001fibo \
