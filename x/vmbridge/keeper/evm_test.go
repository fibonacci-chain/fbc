...
import (
	...
	keeper2 "github.com/fibonacci-chain/fbc/x/vmbridge/keeper"
...
func (suite *KeeperTestSuite) TestSendToWasmEventHandler_Handle() {
	contractAccAddr, err := sdk.AccAddressFromBech32("fb1fnkz39vpxmukf6mp78essh8g0hrzp3gy9zqj7f")
	suite.Require().NoError(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				wasmAddrStr := suite.wasmContract.String()
				input, err := getSendToWasmEventData(wasmAddrStr, "fb111", big.NewInt(1))
				suite.Require().NoError(err)
				data = input
			},
			func() {
