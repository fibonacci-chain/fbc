...
import (
	...
	"github.com/fibonacci-chain/fbc/libs/cosmos-sdk/crypto"
	"github.com/fibonacci-chain/fbc/libs/cosmos-sdk/crypto/keys/hd"
...
// CreateMnemonicWithHDPath generates a new key with the given algorithm and language pair.
func (kb baseKeybase) CreateMnemonicWithHDPath(
	keyWriter keyWriter, name string, language Language, passwd string, algo SigningAlgo, mnemonicInput string, hdPath string,
) (info Info, mnemonic string, err error) {
	...
	info, err = kb.CreateAccount(keyWriter, name, mnemonic, DefaultBIP39Passphrase, passwd, hdPath, algo)
	if err != nil {
		return nil, "", err
	}
