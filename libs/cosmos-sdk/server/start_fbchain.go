...
import (
	...
	mpttypes "github.com/fibonacci-chain/fbc/libs/cosmos-sdk/store/mpt/types"
	sdkstoretypes "github.com/fibonacci-chain/fbc/libs/cosmos-sdk/store/types"
	...
	cmn "github.com/fibonacci-chain/fbc/libs/tendermint/libs/os"
...
type serverHofiboable struct {
	hofiboable map[string]interface{}
}

var gSrvHofiboable = serverHofiboable{make(map[string]interface{})}

func InstallHookEx(flag string, hooker fnHookstartInProcess) {
	gSrvHofiboable.hofiboable[flag] = hooker
}
	...
	case FlagHookstartInProcess:
		{
			...
			function, ok := gSrvHofiboable.hofiboable[FlagHookstartInProcess]
			if !ok {
				return nil
			}
...
set --node-mode=val to manage the following flags:
	--disable-checktx-mutex=true
	--disable-query-mutex=true
	--dynamic-gp-mode=2
	--iavl-enable-async-commit=true
