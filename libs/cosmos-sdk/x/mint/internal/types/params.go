...
var (
	...
	FiboChainTotalMint      = uint64(65777777)
	FiboChainMaxMintYear    = uint64(77)
	FiboChainDeflationEpoch = uint64(1)
	FiboChainDeflationRate  = sdk.NewDecWithPrec(8, 1)
)
...
func DefaultParams() Params {
	return Params{
		...
		BlocksPerYear:  uint64(60 * 60 * 8766 / 3), // assuming 3 second block times //10519200
		DeflationRate:  sdk.NewDecWithPrec(5, 1),   //0.5
		DeflationEpoch: 3,                          // 3 years
		FarmProportion: sdk.NewDecWithPrec(0, 0),   // 0
	}
}
...
func (p Params) String() string {
  ...
  Farm Proportion:                %s
`,
		p.MintDenom, p.DeflationEpoch, p.DeflationRate, p.BlocksPerYear, FiboChainTotalMint, FiboChainMaxMintYear, p.FarmProportion,
	)
}
...
func validateTotalMint(i interface{}) error {
	v, ok := i.(uint64)
	if !ok {
		return fmt.Errorf("invalid parameter type: %T", i)
	}
	...
	if v == 0 {
		return fmt.Errorf("total mint be positive: %d", v)
	}
...
func validateMaxMintYear(i interface{}) error {
	v, ok := i.(uint64)
	if !ok {
		return fmt.Errorf("invalid parameter type: %T", i)
	}
	...
	if v == 0 {
		return fmt.Errorf("max mint year must be positive: %d", v)
	}
