...
import (
	...
	types3 "github.com/fibonacci-chain/fbc/app/types"
	...
	simapp2 "github.com/fibonacci-chain/fbc/libs/ibc-go/testing/simapp"
	...
	types4 "github.com/fibonacci-chain/fbc/x/token/types"
	...
	for i := 0; i < 10; i++ {
		chain.acc[i] = &types3.EthAccount{
			BaseAccount: &auth.BaseAccount{
				Address: env.addr[i],
				Coins:   sdk.Coins{sdk.NewInt64Coin("fibo", 1000000)},
			},
			//CodeHash: []byte{1, 2},
		}
...
func createCosmosTx(t *testing.T, chain *Chain, i int) []byte {
	msg := types4.NewMsgTokenSend(chain.addr[i], chain.addr[i+1], sdk.Coins{sdk.NewInt64Coin("fibo", 10)})
