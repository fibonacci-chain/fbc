...
import (
	...
	srvconfig "github.com/fibonacci-chain/fbc/libs/cosmos-sdk/server/config"
...
var mnemonicList = []string{
	"fragile shoot cover radar escape worry item army script vast clean truly",
	"canal crew list answer close air gravity mushroom bridge curtain stuff motor",
	"boring heavy sport reduce maple prefer pelican smoke mom clean flag vault",
	"hole action sketch muscle post hub ginger cheap clog able license gold",
}
...
func GenerateSaveCoinKey(keybase keys.Keybase, keyName, keyPass string, overwrite bool, algo keys.SigningAlgo, mnemonic string) (sdk.AccAddress, string, error) {
	...
	info, secret, err := keybase.CreateMnemonicWithHDPath(keyName, keys.English, keyPass, algo, mnemonic, hdPath)
	if err != nil {
